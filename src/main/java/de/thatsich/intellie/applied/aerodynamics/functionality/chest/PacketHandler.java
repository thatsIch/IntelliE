...
public enum PacketHandler
{
	...
	/**
	 ...
	 handle {@link cpw.mods.fml.common.network.internal.FMLProxyPacket} but general purpose objects can't be
	 handled sadly.

	 This method uses the {@link IronChestCodec} to transform a custom packet
	 {@link IronChestMessage} into an {@link cpw.mods.fml.common.network.internal.FMLProxyPacket} by using the
	 utility method {@link FMLEmbeddedChannel#generatePacketFrom(Object)} on
	 the channel to do exactly that.

	 @param tileEntityIronChest TileEntity

	 @return Packet
	 */
	public static Packet getPacket( ChestTileEntity tileEntityIronChest )
	{
	...
	/**
	 ...
	 {@link cpw.mods.fml.common.network.internal.FMLProxyPacket} which wraps the client and server custom payload
	...
	private class IronChestCodec extends FMLIndexedMessageToMessageCodec<IronChestMessage>
	{
		...
		public void encodeInto( ChannelHandlerContext ctx, IronChestMessage msg, ByteBuf target ) throws Exception
		{
			...
			int typeAndFacing = ( ( msg.facing & 0x0F ) << 4 ) & 0xFF;
